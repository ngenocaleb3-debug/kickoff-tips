...
import { Zap, Check, X as XIcon, TrendingUp, Lock, Send, LogOut, Trash2 } from "lucide-react";
import { adminLogin, adminLogout } from "./firebase";
...
export function TipCard({ tip, isNew }) {
  ...
  return (
    ...
    }export function TrackRecord({ tips }) {
...
export function AdminPanel({ tips, adminUser, onPublish, onDelete, onSettle }) {
  ...
  return (
    <div className="mx-5 mb-3 flex flex-col gap-3 rounded-2xl p-4" style={{ backgroundColor: COLORS.card, border: `1px solid ${COLORS.cardLine}` }}>
      ...
      <input placeholder="League (e.g. Premier League)" value={form.league} onChange={(e) => update("league", e.target.value)} className="rounded-lg px-3 py-2 text-xs outline-none" style={inputStyle} />
